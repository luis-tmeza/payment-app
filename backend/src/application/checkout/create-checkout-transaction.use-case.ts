...
import { CHECKOUT_REPOSITORY, CheckoutRepository, DuplicateCheckoutError, StockUnavailableError, StoredCheckout } from '../ports/checkout.repository';
...
export class CreateCheckoutTransactionUseCase {
  ...
  async execute(command: CheckoutCommand): Promise<Result<CheckoutResult, CheckoutError>> {
    const existing = await this.checkoutRepository.findByCheckoutKey(command.checkoutKey);
    if (existing) return ok(this.toResult(existing));
    ...
    let pending;
    try {
      pending = await this.checkoutRepository.createPending({ checkoutKey: command.checkoutKey, reference: `PAY-${randomUUID()}`, items, baseFeeCents, deliveryFeeCents, customer: command.customer, delivery: command.delivery, cardBrand: this.cardBrand(command.card.number), cardLastFour: command.card.number.replace(/\D/g, '').slice(-4) });
    } catch (error) {
      if (error instanceof StockUnavailableError) return err('OUT_OF_STOCK');
      if (error instanceof DuplicateCheckoutError) {
        const duplicate = await this.checkoutRepository.findByCheckoutKey(command.checkoutKey);
        if (duplicate) return ok(this.toResult(duplicate));
      }
      throw error;
    }
    ...
    try {
      ...
      await this.checkoutRepository.complete({ transactionId: pending.transactionId, status: 'ERROR', statusMessage: 'Payment gateway request failed' });
      return err('PAYMENT_GATEWAY_ERROR');
    }
  }

  private toResult(checkout: StoredCheckout): CheckoutResult { return { reference: checkout.reference, status: checkout.status, statusMessage: checkout.statusMessage, totalAmountCents: checkout.totalAmountCents }; }
