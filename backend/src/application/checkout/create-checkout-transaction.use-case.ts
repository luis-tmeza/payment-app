...
export class CreateCheckoutTransactionUseCase {
  ...
  async execute(command: CheckoutCommand): Promise<Result<CheckoutResult, CheckoutError>> {
    const product = await this.checkoutRepository.findProduct(command.productId);
    if (!product) return err('PRODUCT_NOT_FOUND');
    if (product.stock < command.quantity) return err('OUT_OF_STOCK');
    ...
    try {
      const payment = await this.paymentGateway.charge({
        ...
        amountInCents: pending.totalAmountCents,
        customer: command.customer,
        card: command.card,
        acceptedTerms: command.acceptedTerms,
        acceptedPersonalData: command.acceptedPersonalData,
        clientIp: command.clientIp,
      });
      const completion = await this.checkoutRepository.complete({
      ...
      return ok({
        ...
        status: payment.status,
        statusMessage: payment.statusMessage,
        totalAmountCents: pending.totalAmountCents,
      });
    } catch {
      await this.checkoutRepository.complete({
        transactionId: pending.transactionId,
        status: 'ERROR',
        statusMessage: 'Payment gateway request failed',
      });
      return err('PAYMENT_GATEWAY_ERROR');
    }
  }

  private cardBrand(cardNumber: string): string | undefined {
