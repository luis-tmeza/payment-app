...
export type CreatePendingCheckout = { reference: string; items: CheckoutProductLine[]; baseFeeCents: number; deliveryFeeCents: number; customer: CheckoutCustomer; delivery: CheckoutDelivery; cardBrand?: string; cardLastFour: string; };
...
export interface CheckoutRepository { findProducts(productIds: string[]): Promise<Product[]>; createPending(input: CreatePendingCheckout): Promise<PendingCheckout>; findByReference(reference: string): Promise<StoredCheckout | null>; complete(input: CompleteCheckout): Promise<'COMPLETED' | 'OUT_OF_STOCK'>; }
