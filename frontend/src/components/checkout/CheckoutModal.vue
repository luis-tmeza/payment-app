    ...
    <div v-if="isOpen" class="checkout-backdrop" @mousedown.self="close">
      <section class="checkout-modal" role="dialog" aria-modal="true" aria-labelledby="checkout-title">
        <header class="checkout-header">
          <div><p class="eyebrow">Checkout seguro</p><h2 id="checkout-title">{{ isSummary ? 'Revisa antes de pagar' : 'Completa tu pago' }}</h2></div>
          <button class="icon-button" type="button" aria-label="Cerrar checkout" @click="close"><X :size="20" /></button>
        </header>

        <form v-if="!isSummary" class="checkout-form checkout-form--structured" @submit.prevent="continueToSummary">
          ...
          <aside class="checkout-order-context" aria-label="Resumen de la compra"><img v-if="product.imageUrl" :src="product.imageUrl" :alt="product.name" /><div><span>Orden actual</span><strong>{{ items.length > 1 ? `${items.length} productos` : product.name }}</strong><small>{{ formatMoney(productSubtotalCents) }} en productos</small></div><strong class="checkout-order-total">{{ formatMoney(totalCents) }}</strong></aside>
          ...
          <div class="checkout-form-layout">
            ...
            <fieldset class="form-section delivery-section">
              <legend><MapPin :size="18" /> Datos de entrega</legend>
              <div class="form-grid">
                ...
                <label class="field">Tipo de documento<select v-model="form.documentType" required><option value="CC">Cedula de ciudadania</option><option value="CE">Cedula de extranjeria</option><option value="PP">Pasaporte</option></select></label>
          ...
          <div class="checkout-consents"><label class="consent-field"><input v-model="form.acceptedTerms" type="checkbox" required /><span>Acepto los <a v-if="acceptanceDocuments" :href="acceptanceDocuments.termsUrl" target="_blank" rel="noopener">terminos y condiciones de Wompi</a><template v-else>terminos y condiciones de Wompi</template>.</span></label><label class="consent-field"><input v-model="form.acceptedPersonalData" type="checkbox" required /><span>Autorizo el <a v-if="acceptanceDocuments" :href="acceptanceDocuments.personalDataUrl" target="_blank" rel="noopener">tratamiento de mis datos personales</a><template v-else>tratamiento de mis datos personales</template>.</span></label></div>
          <p v-if="formError" class="form-error" role="alert">{{ formError }}</p>
          <footer class="checkout-form-footer"><p class="payment-assurance"><ShieldCheck :size="17" aria-hidden="true" /> La informacion de tu tarjeta se procesa de forma segura.</p><button class="pay-button checkout-submit" type="submit">Continuar a confirmacion</button></footer>
        </form>
        ...
        <div v-else class="checkout-summary">
          ...
          <div class="summary-product"><img v-if="product.imageUrl" :src="product.imageUrl" :alt="product.name" /><div><span>Orden</span><strong>{{ items.length > 1 ? `${items.length} productos` : product.name }}</strong><small>Tarjeta {{ cardBrand || 'credito' }} terminada en {{ lastFour }}</small></div></div>
          <dl class="summary-lines"><div><dt>Productos ({{ items.length }})</dt><dd>{{ formatMoney(productSubtotalCents) }}</dd></div><div><dt>Tarifa base</dt><dd>{{ formatMoney(baseFeeCents) }}</dd></div><div><dt>Envio</dt><dd>{{ formatMoney(deliveryFeeCents) }}</dd></div><div class="summary-total"><dt>Total a pagar</dt><dd>{{ formatMoney(totalCents) }}</dd></div></dl>
          <div class="summary-actions"><button class="secondary-button" type="button" :disabled="isProcessing" @click="isSummary = false">Editar datos</button><button class="pay-button" type="button" :disabled="isProcessing" @click="emitPayment">{{ isProcessing ? 'Procesando pago...' : 'Pagar ahora' }}</button></div>
...
import { CreditCard, MapPin, ShieldCheck, X } from 'lucide-vue-next';
