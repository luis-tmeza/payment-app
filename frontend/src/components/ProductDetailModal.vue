<template>
  <Teleport to="body">
    <div v-if="isOpen" class="product-detail-backdrop" @mousedown.self="emit('close')">
      <section class="product-detail-modal" role="dialog" aria-modal="true" aria-labelledby="product-detail-title">
        <button class="icon-button product-detail-close" type="button" aria-label="Cerrar detalle" @click="emit('close')"><X :size="20" /></button>
        <div class="product-detail-media"><img v-if="product.imageUrl" :src="product.imageUrl" :alt="product.name" /><div v-else class="image-placeholder" aria-hidden="true"></div></div>
...
import { ArrowRight, RotateCcw, ShieldCheck, ShoppingCart, Truck, X } from 'lucide-vue-next';
import type { Product } from '../types/product';
defineProps<{ product: Product; isOpen: boolean }>();
const emit = defineEmits<{ close: []; buy: []; 'add-to-cart': [] }>();
