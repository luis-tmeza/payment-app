...
const products = ref<Product[]>([]); const selectedProduct = ref<Product | null>(null); const checkoutItems = ref<CartItem[]>([]); const isCartCheckout = ref(false); const isDetailOpen = ref(false); const isCartOpen = ref(false); const isLoading = ref(true); const loadError = ref(false); const isProcessing = ref(false); const paymentError = ref(''); const paymentResult = ref<CheckoutTransactionResponse | null>(null); const acceptanceDocuments = ref<AcceptanceDocuments | null>(null);
...
const handlePayment = async (paymentData: CheckoutPaymentData): Promise<void> => { if (!selectedProduct.value) return; isProcessing.value = true; paymentError.value = ''; try { paymentResult.value = await createCheckoutTransaction(checkoutItems.value, paymentData); if (isCartCheckout.value && paymentResult.value.status === 'APPROVED') store.commit('clearCart'); store.commit('setTransactionResult', paymentResult.value); } catch { paymentError.value = 'No fue posible procesar el pago. Verifica tus datos e intentalo nuevamente.'; } finally { isProcessing.value = false; } };
const returnToProduct = async (): Promise<void> => { paymentResult.value = null; checkoutItems.value = []; isCartCheckout.value = false; store.commit('reset'); await loadProducts(); };
const startCheckout = (items: CartItem[], fromCart: boolean): void => { if (!items.length || items.some((item) => item.stock < item.quantity)) return; selectedProduct.value = items[0]; checkoutItems.value = items; isCartCheckout.value = fromCart; closeDetails(); void getAcceptanceDocuments().then((documents) => { acceptanceDocuments.value = documents; }).catch(() => { acceptanceDocuments.value = null; }); store.commit('selectProduct', selectedProduct.value.id); };
