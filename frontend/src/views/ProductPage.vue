<template>
  <main class="storefront">
    ...
    <ProductDetailModal v-if="selectedProduct" :is-open="isDetailOpen" :product="selectedProduct" @close="closeDetails" @add-to-cart="addProductToCart(selectedProduct)" @buy="buyProduct(selectedProduct)" />
    <CartDrawer :is-open="isCartOpen" :items="store.state.cart" :subtotal-cents="cartSubtotalCents" @close="isCartOpen = false" @increase="increaseCartItem" @decrease="decreaseCartItem" @remove="removeFromCart" @checkout="checkoutCart" />
...
const products = ref<Product[]>([]); const selectedProduct = ref<Product | null>(null); const checkoutItems = ref<CartItem[]>([]); const checkoutKey = ref(''); const isCartCheckout = ref(false); const isDetailOpen = ref(false); const isCartOpen = ref(false); const isLoading = ref(true); const loadError = ref(false); const isProcessing = ref(false); const paymentError = ref(''); const paymentResult = ref<CheckoutTransactionResponse | null>(null); const acceptanceDocuments = ref<AcceptanceDocuments | null>(null);
...
const buyProduct = (product: Product): void => { selectProduct(product); startCheckout([{ ...product, quantity: 1 }], false); };
